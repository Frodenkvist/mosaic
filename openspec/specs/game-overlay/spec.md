# game-overlay Specification

## Purpose

The game-overlay capability provides an in-game, transparent, click-through presentation surface drawn over games launched through Mosaic. It is tied to a Mosaic-launched play session: created when the session starts and torn down when it ends. While idle it is invisible and non-interactive (no focus, no input capture, absent from the taskbar and Alt-Tab), and it surfaces achievement unlocks as auto-dismissing toasts accompanied by an optional audio cue. The overlay and the sound are independently configurable and default to enabled; when the overlay cannot composite over the game (e.g. exclusive fullscreen) the unlock is never lost — the audio cue and the existing in-application notification still convey it.
